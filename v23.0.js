// SAIKO Construction AI v23.0 — Project subcontract dependency category
(function(){
  ...
  function fillProjectForm(p){
    ...
    if($('pSubconDependency')) $('pSubconDependency').value=p?.subcon_dependency||'Independent';
  }
  ...
  $('addProjectBtn').onclick=()=>{
    ...
    $('pDiscount').value=0;
    if($('pSubconDependency')) $('pSubconDependency').value='Independent';
    if(typeof updateProjectNet==='function')updateProjectNet();
  ...
  renderProjects=function(){
    ...
    $('projectRows').innerHTML=rows.length?rows.map(p=>`<tr>
      ...
      <td>${esc(p.status)}</td>
      <td><span class="project-subcon-badge ${p.subcon_dependency==='Dependent'?'dependent':'independent'}">${esc(p.subcon_dependency||'Independent')}</span></td>
    ...
    </tr>`).join(''):'<tr><td colspan="9" class="empty">No projects yet.</td></tr>';
  ...
  $('projectForm').onsubmit=async e=>{
    ...
    const values={
      ...
      subcon_dependency:$('pSubconDependency')?.value||'Independent',
