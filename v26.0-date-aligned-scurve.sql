...
-- Stores one actual cumulative accomplishment point per project/date
-- so Dashboard can compare Actual vs Projected on the same date axis.
...
begin
  if to_regprocedure('public.bump_system_release(text)') is not null then
    perform public.bump_system_release('v26.0 date-aligned projected vs actual S-curve');
  end if;
end $$;

select 'v26.0 ready' as result;
